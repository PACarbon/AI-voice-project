...
char g_rec_result[BUFFER_SIZE] = {0};
...
void run_iat(const char *audio_file, const char *session_begin_params)
{
	...
	printf("%s\n", rec_result);
	printf("=============================================================\n");
	strcpy(g_rec_result, rec_result);
...
{
	int ret = MSP_SUCCESS;
	// TODO：重点 appid = 6f950767, work_dir = .  // 登录参数，appid与msc库绑定,请按照自己的id来
...
void my_iat()
{
	...
	run_iat("/mnt/hgfs/AI_Server/2.wav", session_begin_params); 
}
// 我自己的语音合成
// 我自己的语音合成
